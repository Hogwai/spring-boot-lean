...
build-docker-micronaut:
	docker build -f micronaut/Dockerfile -t spring-lean:micronaut ./micronaut
...
clean:
	mvn clean
	cd micronaut && mvn clean 2>/dev/null || true
	cd rust && cargo clean 2>/dev/null || true
	docker rmi spring-lean:jvm spring-lean:native spring-lean:go spring-lean:rust spring-lean:micronaut 2>/dev/null || true
	rm -rf target/
	rm -f /tmp/golean
...
help:
	...
	@echo "  run-micronaut      - Run Micronaut locally (mvn mn:run)"
